...
$(".music-list").on("click",".icon-play,.icon-download,.icon-share", function() {
    switch($(this).data("function")) {
        case "play": // 播放
            var num = parseInt($(this).parents(".list-item").data("no"));
        	//如果当前地址为空，或者点击的歌曲与当前不一样或者处于搜索后第一次点击播放，那么选择当前点击的歌曲播放
            var flag = krAudio.Currentplay == num ? false : true;
            // $(this).parents(".list-item").index()
        	if(isEmpty(krAudio.audioDom.src) || flag ){
        		krAudio.Currentplay = num;
                // $(this).parents(".list-item").get//当前播放的音乐序号
        		listMenuStyleChange(krAudio.Currentplay);  //列表菜单的播放暂停按钮的变
                krAudio.seturl();
				krAudio.play();
        	}
...
function appendlistMenu(){
	$(".songlist__item .songlist__item--even").each(function(index,el) {
	    var target = $(el).find(".songlist__songname");
...
function clearDislist() {// 清空内容
    // $.ajax(
    //     {
    //         url:"http://localhost:8080/playSong/clear"
    //     }
    // )
    // layer.msg('列表已被清空');
}
