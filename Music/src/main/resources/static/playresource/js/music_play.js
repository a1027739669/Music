...
var krAudio = {
	...
	seturl:function(){
		...
		var currentObject =$(".list-item")[this.Currentplay-1];
			// $("#main-list .list-item").eq(this.Currentplay-1);
		//获取当前播放对象
		var url =currentObject.getAttribute("data-url");
	...
	play:function(){
		...
		if(this.changeURL){ //搜索标志位是false和播放地址改变才会进入
			var currentObject = $(".list-item")[this.Currentplay-1];
				// $("#main-list .list-item").eq(this.Currentplay-1); //获取当前播放对象
			currentObject=jQuery(currentObject);
