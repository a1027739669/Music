            ...
            <#if commentPage ? exists>
                <#list commentPage.content as comment>
                    <li class="comment__list_item c_b_normal js_cmt_li" style="visibility: visible;">
                        <a href="/guest/lokeother?userId=${comment.getUser().getId()}" class="comment__avatar js_nick"
                           data-uin="1280312987">
                        ...
                        <h4 class="comment__title"><a href="/guest/lokeother?userId=${comment.getUser().getId()}"
                                                      class="c_tx_thin js_nick js_nick_only"
                                                      data-uin="1280312987">${comment.getUser().nicheng}</a>
        ...
        <div class="mod_page_nav js_pager_comment">
            ...
            <#elseif (commentPage.totalPages>1)&&(commentPage.totalPages<=4)>
                <#if ((commentPage.getNumber()+1)!=1)>
                    <a href="javascript:;" class="next js_pageindex" title="上一页"
                       hidefocus="" onclick="toPage(${commentPage.getNumber()},${song.songId})"><span>&lt;</span></a>
                </#if>
                <#list 1..commentPage.totalPages as curindex>
                    ...
                    <#else >
                        <a href="javascript:;" class="js_pageindex" hidefocus=""
                           onclick="toPage(${curindex},${song.songId})">${curindex}</a>
                    </#if>
                <#else>
                    <strong class="current">${curindex}</strong>
                </#list>
                <#if ((commentPage.getNumber()+1)!=commentPage.totalPages)>
                    <a href="javascript:;" class="next js_pageindex" title="下一页"
                       hidefocus="" onclick="toPage(${commentPage.getNumber()+2},${song.songId})"><span>&gt;</span></a>
                </#if>
            <#elseif (commentPage.totalPages>=5)&&(commentPage.totalPages<9)>
                ...
                <#if (commentPage.getNumber()>3)>
                    <a href="javascript:;" class="js_pageindex" hidefocus="" onclick="toPage(1,${song.songId})">1</a>
                ...
                <#else>
                    ...
                    <strong class="more">...</strong>
                    <a href="javascript:;" class="js_pageindex" hidefocus="" onclick="toPage(9,${song.songId})">9</a>
                </#if>
                <#if ((commentPage.getNumber()+1)!=commentPage.totalPages)>
                    <a href="javascript:;" class="next js_pageindex" title="下一页"
                       hidefocus="" onclick="toPage(${commentPage.getNumber()+2},${song.songId})"><span>&gt;</span></a>
            ...
            <#elseif (commentPage.totalPages>=9)>
                ...
                <#if ((commentPage.getNumber()+1)<=4)>
                    ...
                    <strong class="more">...</strong>
                    <a href="javascript:;" class="js_pageindex" hidefocus=""
                       onclick="toPage(${commentPage.totalPages},${song.songId})">${commentPage.totalPages}</a>
...
<script type="text/javascript">
    $(document).ready(function () {
        initBind();
        if($("#reply_text_blur").text().length>300){
            $("#reply_text_blur").text($("#reply_text_blur").text().substring(0,300));
        }
    });

    function initBind() {
        $("#reply_text_blur").blur(function () {
            checkMessage();
        });
        $("#reply_text_blur").keyup(function () {
            checkMessage();
        });
        $("#reply_text_blur").keydown(function () {
            checkMessage();
        });
    }

    function checkMessage() {
        $("#remain").html(300-$('#reply_text_blur').text().length );
    }
